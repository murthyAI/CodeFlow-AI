...
# --- DATABASE LOGISTICS SYSTEM (v14.6 CORE) ---
...
if not row:
    db.execute("INSERT INTO users VALUES (?, 263605, 900, 1, '', 0, 500, '', 0, 'Basic Tractor')", (USER_ID,))
    conn.commit()
    coins, pph, level, last_claim, streak_count, energy, wallet_address, total_invites, tractor_tier = 263605, 900, 1, "", 0, 500, "", 0, "Basic Tractor"
else:
    coins, pph, level, last_claim, streak_count, energy, wallet_address, total_invites, tractor_tier = row

# --- BACKEND AUTO ENERGY RECHARGE LOGIC ---
if energy is None or energy < 500:
    energy = 500  # Automatic fallback full-recharge on session load/refresh
    db.execute("UPDATE users SET energy = 500 WHERE id = ?", (USER_ID,))
    conn.commit()

# --- SESSION STATE LOCKS FOR ANIMATIONS & TASKS ---
...
# --- MULTIPLIER BASED ON TRACTOR SKINS ---
...
# --- NAVIGATION PANEL DISTRIBUTION LOGIC ---
if active_panel == "🎯 MINE":
    ...
    if st.button("⚡ TAP TRACTOR TO HARVEST COINS ⚡", key="harvest_v146", use_container_width=True):
        if energy >= 10:
            energy -= 10
    ...
    # 1. MYSTERY CRATE GAME COMPONENT
    st.markdown("""
    ...
    if st.button("🔓 Open Premium Mystery Box (1,000 Coins)", key="crate_v146", use_container_width=True):
        if coins >= 1000:
            coins -= 1000
    ...
    # 2. TRACTOR PREMIUM SKIN SHOP COMPONENT
    ...
    if tractor_tier != "Basic Tractor":
        ...
        if st.button("Buy Iron Tractor Skin", key="buy_iron", use_container_width=True):
            if coins >= 50000:
                ...
                conn.commit()
                st.success("Successfully upgraded to Iron Tractor Skin! Double mining activated!")
                st.rerun()
            else:
                st.error("❌ Insufficient coins for upgrade!")
    ...
    if tractor_tier == "Cyber Tractor":
        ...
        if st.button("Buy Cyber Gold Tractor Skin", key="buy_cyber", use_container_width=True):
            if coins >= 150000:
                ...
                conn.commit()
                st.success("Successfully upgraded to Cyber Gold Tractor Skin! x4 multiplier loaded!")
                st.rerun()
            else:
                st.error("❌ Insufficient coins for upgrade!")
    ...
    # Render progressive streak grid array setup
    ...
    if last_claim == current_date_stamp:
        st.warning(f"🔒 Streak day {streak_count} claimed successfully! Return tomorrow safely.")
    else:
    ...
    # 2. SOCIAL MEDIA MISSIONS COMPONENT BLOCK
    ...
    # DISPLAY TOTAL ACTIVE INVITATION COUNTER DATA
    st.markdown(f"""
    ...
    # COPY SYSTEM ROUTE PLACEHOLDERS
    ...
    if st.button("🔗 SHARE REFERRAL LINK", use_container_width=True):
        st.toast("Referral code copied seamlessly to dashboard clipboard!", icon="🚀")
        
    st.markdown("<br>", unsafe_allow_html=True)
    
    # SIMULATION SIMULATOR ENGINE (FOR DEMO/TEST FEEDBACK)
    if st.button("👥 SIMULATE 1 SUCCESSFUL INVITE (+25,000 Coins)", key="simulate_invite_btn", use_container_width=True):
        ...
        conn.commit()
        st.balloons()
        st.success("Simulation Success! 1 Fren Joined. 🪙 +25,000 Coins loaded inside wallet node!")
        st.rerun()
    ...
    if wallet_address and wallet_address.strip() != "":
        # Wallet is already linked setup configuration block locked
        st.markdown(f"""
            <div style="background: linear-gradient(135deg, rgba(76,175,80,0.12) 0%, rgba(0,0,0,0.8) 100%); border: 2px solid #4caf50; border-radius: 20px; padding: 25px; text-align: center; box-shadow: 0 0 25px rgba(76,175,80,0.25);">
                ...
                <p style="font-size:11px; color:#aaa; margin:5px 0 0 0;">Token snapshot distribution protocol status: ELIGIBLE. Simulated transaction verification task unlockable 1-month prior to listing (August 2026).</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        # Live Text box Input field unlocked ready to save inside DB
        st.markdown("""
            <p style="font-size:0.8rem; color:#ccc;">Submit your standard non-custodial TON Wallet node key configuration (e.g. Tonkeeper, MyTonWallet) to lock your allocation data node prior to snapshot deadline schedules.</p>
        """, unsafe_allow_html=True)
        
        input_address = st.text_input("Enter TON Wallet Node Address Link", placeholder="EQA1b...7z3X_N7pG2qK", key="wallet_input_box")
        
        if st.button("🔗 LINK WALLET NODE ADDRESS", use_container_width=True, key="save_wallet_address_v146"):
            if input_address.strip() != "":
                db.execute("UPDATE users SET wallet_address = ? WHERE id = ?", (input_address.strip(), USER_ID))
                conn.commit()
                st.success("TON Wallet node reference bound successfully to master snapshot ledger!")
                st.rerun()
            else:
                st.error("❌ Box cannot be submitted blank! Provide a valid configuration trace.")

# --- REVENUE MONETIZATION PROMO BANNER ---
st.markdown("""
